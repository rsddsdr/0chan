<template>
    <div v-if="!(!tree && post.isDeleted && !post.canBeModerated)">
        ...
        <div style="margin-left: 25px;" v-if="post.canReplyTo">
            <PostForm v-if="isReplyFormShown" :reply-to="post" :board="thread.board" @submit="onReply" ref="form"></PostForm>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            ratePost(isLike=false) {
                Post.rate(this.post.id, isLike).then(response => {
                    if (response.data.ok) {
                        if (isLike) {
                            this.post.likes.liked += 1;
                        } else {
                            this.post.likes.disliked += 1;
                        }
                    } else {
                        switch(response.data.error) {
                            case 'aleardy_voted':
                                if (response.data.cancel) {
                                    this.$bus.emit(BusEvents.ALERT_SUCCESS, "Вы отменили свой голос") ;
                            ...
                            case 'likes_disabled':
                                this.$bus.emit(BusEvents.ALERT_ERROR, "Рейтинг на этой доске отключен") ;
                                this.thread.board.likes = false;
                                break;

                            case 'op_match':
                                this.$bus.emit(BusEvents.ALERT_ERROR, "Нельзя ставить оценки на свой пост") ;
                                break;
                        }
                    }
                }, error => {
            ...
            replyToPost(post_id) {
                this.$bus.emit(BusEvents.REPLY_TO_POST, post_id);
            },
            restorePost() {
                this.post.isDeleted = null;
